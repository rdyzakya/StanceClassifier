...
def init_args():
    ...
    # src_path, output_path, label_path
    parser.add_argument("--src_path", type=str, default="data/processed/processed_data.csv")
    parser.add_argument("--output_path", type=str, default="data/processed/processed_data_bio.txt")
    parser.add_argument("--label_path", type=str, default="data/processed/labels.txt")
...
def create_bio_label(row,labels,tokenizer):
    text = row["content"]
...
def main():
    ...
    tokenized_dataset = dataset.map(lambda x : create_bio_label(x,labels=labels,tokenizer=tokenizer), batched=False, remove_columns=dataset.column_names)
    ...
    with open(output_path, "w") as f:
        for i in range(len(tokenized_dataset)):
            for j in range(len(tokenized_dataset[i]["tokens"])):
                if tokenized_dataset[i]["tokens"][j].strip() == "":
                    continue
